...
public class CameraUtil {
    ...
    private static void createAlbumP(AppCompatActivity activity,int num,onSelectCallback callback){
        EasyPhotos.createAlbum(activity, true, true, GlideEngine.getInstance())
...
//                .setCount(num>0?num:1)
     ...
     * @param callback 回调
     */
    public static void selectVideo(AppCompatActivity activity,int num,onSelectCallback callback){
        EasyPhotos.createAlbum(activity, false, true, GlideEngine.getInstance())
                .setFileProviderAuthority(activity.getPackageName()+".fileprovider")
